    ...
    <>
              ...
              <div className="lg:sticky lg:top-28">
                ...
                <div className="mt-10 rounded-2xl bg-brand-violet-soft p-5 ring-1 ring-brand-primary/5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-accent text-brand-primary font-bold">
                      P
                    </div>
                    <div>
              ...
              <div className="prose-pd">
                ...
                <div className="my-10 rounded-[1.75rem] bg-brand-primary p-7 text-white shadow-card md:p-9">
                  ...
                  <p className="mt-3 text-[15px] leading-relaxed text-white/75">
                    Agende uma avaliação dermatológica especializada na PetDerma. Identificamos a
                    causa raiz e definimos o tratamento certo desde a primeira consulta.
                  </p>
                  <div className="mt-5">
                    <Magnetic>
                      <WhatsAppCTA
                ...
                {post.faq && post.faq.length > 0 && (
                  <div className="mt-12">
                    ...
                    <Accordion.Root
                      type="single"
                      collapsible
                      className="mt-6 divide-y divide-brand-primary/10"
                    >
                      {post.faq.map((item, i) => (
      ...
      <section className="py-12">
        <Container size="default" className="text-center">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 rounded-pill border border-brand-primary/15 bg-white px-6 py-3 text-sm font-semibold text-brand-primary transition-all hover:border-brand-accent hover:bg-brand-accent"
          >
  ...
  switch (block.type) {
    case "h2":
      return (
        <h2
          id={block.id}
          className="mt-12 scroll-mt-32 font-display text-2xl font-bold tracking-tight text-brand-primary md:text-3xl"
        >
          {block.text}
        </h2>
      );
    case "h3":
      return (
        <h3
          id={block.id}
          className="mt-8 scroll-mt-32 font-display text-xl font-semibold tracking-tight text-brand-primary md:text-2xl"
        >
          {block.text}
        </h3>
      );
      ...
      return (
        <ul className="mt-5 space-y-2.5">
          {block.items.map((item, i) => (
            <li
              key={i}
              className="flex items-start gap-3 text-[16px] leading-relaxed text-brand-primary/85"
            >
        ...
        <ol className="mt-5 space-y-3">
          {block.items.map((item, i) => (
            <li key={i} className="flex items-start gap-4 text-[16px] leading-relaxed text-brand-primary/85">
              <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-accent text-xs font-bold text-brand-primary">
                {i + 1}
              </span>
              <span>{item}</span>
            </li>
    ...
    case "callout": {
      ...
      return (
        <div
          className={`my-8 rounded-2xl border-l-4 p-5 md:p-6 ${variants[block.variant]}`}
        >
    ...
    case "image":
      return (
        <figure className="my-8">
          ...
          {block.caption && (
            <figcaption className="mt-3 text-center text-xs text-brand-secondary">
              {block.caption}
            </figcaption>
          )}
        </figure>
...
function RelatedCard({ post }: { post: BlogPost }) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] bg-white shadow-soft ring-1 ring-brand-primary/5 transition-all hover:-translate-y-1 hover:shadow-card"
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        <span className={`inline-flex w-fit rounded-pill ${cat?.cor ?? "bg-brand-mint"} px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-brand-primary`}>
          {cat?.nome ?? "Geral"}
        </span>
        <h3 className="mt-3 font-display text-lg font-bold leading-tight tracking-tight text-brand-primary line-clamp-2">
          {post.title}
        </h3>
        <p className="mt-2 flex-1 text-sm leading-relaxed text-brand-secondary line-clamp-2">
          {post.excerpt}
        </p>
